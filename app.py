# Lab Wash V6.1 - ガーゼ・布の洗浄評価・画像解析アプリ
# 修正: 洗剤濃度→実験条件(自由記述), クリップボード貼り付け対応強化
...
st.set_page_config(page_title="Lab Wash V6.1", page_icon="🧪", layout="wide")
...
def generate_pdf_buffer(metadata,weights,results_df,lab_data,wash_rates,images_dict,calibrated_dict):
    ...
    story=[]
    story.append(Paragraph("<b>Lab Wash V6.1 レポート</b>",styles['Title']))
...
with st.sidebar:
    st.title("🧪 Lab Wash V6.1")
    st.caption("温度・時間・実験条件(自由記述) + 貼り付け強化")
    if HAS_PASTE_BUTTON: st.success("📋 貼り付けボタン有効")
    else: st.warning("貼り付け未導入: requirementsに streamlit-paste-button 追加")
    if st.button("🔄 リセット"):
...
with tab1:
    ...
    <style>#paste-hint{border:2px dashed #4CAF50;border-radius:10px;padding:15px;text-align:center;background:#F1F8E9;font-family:sans-serif}#paste-hint.dragover{background:#DCEDC8}</style>
    <div id="paste-hint" tabindex="0">📋 ここクリック → <b>Ctrl+V / Cmd+V</b> で貼り付け可能 (スクショ直後OK)<br><small>ドラッグ&ドロップもOK。下のボタンでも貼り付け可能</small><div id="paste-preview" style="margin-top:10px;"></div></div>
    <script>
    const hint=document.getElementById('paste-hint'); hint.focus();
    document.addEventListener('paste',async(e)=>{
      for(let item of e.clipboardData.items){
        if(item.type.indexOf('image')!==-1){
          const blob=item.getAsFile(); const url=URL.createObjectURL(blob);
          document.getElementById('paste-preview').innerHTML='<img src="'+url+'" style="max-width:100%;max-height:200px;border:1px solid #ccc;border-radius:5px;"><br><small style="color:green;">✓ 検出! 下の貼り付けボタンか、ファイル選択にドロップしてください</small>';
        }
      }
    });
    </script>
    ...
    for idx,key in enumerate(["素地","洗浄前","洗浄後"]):
        with cols[idx]:
            st.markdown(f"**{key}**")
            up=st.file_uploader(f"{key}ファイル",type=["png","jpg","jpeg","bmp","tiff"],key=f"upload_{key}",label_visibility="collapsed")
            if up:
                try:
            ...
            if HAS_PASTE_BUTTON:
                try:
                    pasted=paste_button(f"📋 貼り付け {key}",key=f"paste_btn_{key}")
                    pil_from=None
                    if pasted is not None:
                        if isinstance(pasted, Image.Image): pil_from=pasted.convert("RGB")
                            ...
                            if isinstance(d, Image.Image): pil_from=d.convert("RGB")
                                ...
                                b64=d.split(",",1)[1]
                                pil_from=Image.open(io.BytesIO(base64.b64decode(b64))).convert("RGB")
                    if pil_from:
                        st.session_state.images[key]=pil_from
                        st.session_state.calibrated_images[key]=None
                        st.session_state.masks[key]=None
                        st.success(f"{key} 貼り付け成功!")
                        st.rerun()
                except Exception as e: st.warning(f"貼り付けエラー: {e}")
            if st.session_state.images[key] is not None:
                st.image(st.session_state.images[key],caption=f"{key}",use_column_width=True)
    ...
    if all(v is None for v in st.session_state.images.values()): st.warning("①で画像登録してください")
        ...
        for key in ["素地","洗浄前","洗浄後"]:
            ...
            with c3:
                ...
                ca,cb=st.columns(2)
                with ca: st.image(pil_img,caption="元",use_column_width=True)
                with cb: st.image(cal,caption="補正後",use_column_width=True)
            st.divider()
        ...
        if use_common:
            ...
            for key in keys:
                ...
                with c1:
                    ...
                    st.image(cv_to_pil(cv_full),caption="ROI",use_column_width=True)
                with c2: st.image(roi,caption="クロップ",use_column_width=True)
                with c3: st.image(binary,caption="二値",use_column_width=True)
                with c4: st.image(overlay,caption="オーバーレイ",use_column_width=True)
        else:
            for key in keys:
                src=st.session_state.calibrated_images.get(key) or st.session_state.images.get(key)
                if src is None: continue
...
with tab5:
    ...
    if st.session_state.get("results_df") is None: st.warning("先に解析実行")
        ...
        if "pdf_buffer" in st.session_state and st.session_state["pdf_buffer"]:
            st.download_button("⬇️ PDFダウンロード",data=st.session_state["pdf_buffer"],file_name=f"{st.session_state.metadata.get('exp_id','report')}_V6.1.pdf",mime="application/pdf")
