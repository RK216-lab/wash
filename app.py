        ...
        for key in ["素地","洗浄前","洗浄後"]:
            ...
            with c3:
                ...
                try:
                    ...
                    with col_a:
                        st.image(pil_img, caption="元画像", use_container_width=True)
                    with col_b:
                        st.image(calibrated, caption=f"補正後 B{black} W{white}", use_container_width=True)
                except Exception as e:
                    st.error(f"補正エラー: {e}")
    ...
    if not has_image:
        ...
        if use_common:
            ...
            for key in ["素地","洗浄前","洗浄後"]:
                ...
                with c3:
                    st.image(binary, caption="二値化", use_container_width=True)
                with c4:
                    st.image(overlay, caption="マスクオーバーレイ", use_container_width=True)
                ...
                with cols[idx]:
                    ...
                    if img:
                        ...
                        if mask is not None:
                            ...
                            st.image(cv_to_pil(blended), caption=f"{key} +マスク", use_container_width=True)
                        else:
